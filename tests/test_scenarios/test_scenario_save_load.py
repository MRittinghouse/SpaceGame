...
class TestSaveLoadCapstonesReached:
    """AC5: capstones_reached round-trips through save/load correctly."""

    def test_populated_set_survives_round_trip(self) -> None:
        player = fresh_player()
        ...
        assert restored.capstones_reached == {"wealth_capstone", "empire_capstone"}, (
            "Populated capstones_reached must survive round-trip exactly"
        )
    ...
    def test_legacy_save_missing_key_yields_empty_set(self) -> None:
        """Legacy save with no capstones_reached key deserializes to empty set."""
        import json
        ...
        mgr = SaveManager()
        data = mgr._serialize_player(player)
        # Simulate legacy save by removing the key.
        data.pop("capstones_reached", None)
        json_str = json.dumps(data)
        restored_data = json.loads(json_str)
        restored = mgr._deserialize_player(restored_data)

        assert restored.capstones_reached == set(), (
            "Missing capstones_reached key must yield empty set for legacy saves"
        )
        assert isinstance(restored.capstones_reached, set)

    def test_malformed_payload_yields_empty_set(self) -> None:
        """Non-list capstones_reached payload deserializes to empty set without crash."""
        import json
        ...
        mgr = SaveManager()
        data = mgr._serialize_player(player)
        # Inject a malformed (non-list) payload.
        data["capstones_reached"] = {"not": "a list"}
        json_str = json.dumps(data)
        restored_data = json.loads(json_str)
        restored = mgr._deserialize_player(restored_data)

        assert restored.capstones_reached == set(), (
            "Malformed (non-list) capstones_reached must yield empty set"
        )
        assert isinstance(restored.capstones_reached, set)
