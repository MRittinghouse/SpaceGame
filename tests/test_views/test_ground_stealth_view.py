...
class TestEnemyRendering:
    ...
    def test_render_with_enemies(self) -> None:
        enemy = GroundEnemy(
            id="guard", x=5, y=5, facing=Direction.RIGHT, vision_range=3
        )
    ...
    def test_render_enemy_only_on_visible_tile(self) -> None:
        ...
        enemy = GroundEnemy(
            id="guard", x=12, y=12, facing=Direction.LEFT, vision_range=3
        )
...
class TestEnemyTurnProcessing:
    ...
    def test_enemy_moves_after_player_move(self) -> None:
        ...
        enemy = GroundEnemy(
            id="guard", x=10, y=5, facing=Direction.RIGHT,
            vision_range=3, patrol_route=route,
        )
        view, mission = _make_stealth_view(
            player_x=2, player_y=2, enemies=[enemy]
        )
...
class TestDetectionIntegration:
    ...
    def test_enemy_detects_player_raises_alert(self) -> None:
        ...
        enemy = GroundEnemy(
            id="guard", x=6, y=2, facing=Direction.LEFT, vision_range=5
        )
        view, mission = _make_stealth_view(
            player_x=2, player_y=2, enemies=[enemy]
        )
    ...
    def test_enemy_does_not_detect_player_behind(self) -> None:
        ...
        enemy = GroundEnemy(
            id="guard", x=5, y=2, facing=Direction.RIGHT, vision_range=5
        )
        view, mission = _make_stealth_view(
            player_x=2, player_y=2, enemies=[enemy]
        )
...
class TestNoiseIntegration:
    ...
    def test_noisy_floor_triggers_suspicion(self) -> None:
        ...
        enemy = GroundEnemy(
            id="guard", x=6, y=6, facing=Direction.DOWN, vision_range=3
        )
    ...
    def test_door_open_generates_noise(self) -> None:
        ...
        enemy = GroundEnemy(
            id="guard", x=7, y=5, facing=Direction.LEFT, vision_range=2
        )
        mission.enemies.append(enemy)
