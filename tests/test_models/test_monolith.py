...
class TestMonolith:
    ...
    def test_monolith_spawns_at_depth_5(self) -> None:
    ...
    def test_monolith_spawns_at_depth_10(self) -> None:
        config = MiningConfig(system_id="breakstone")
        session = MiningSession(config)
        session.depth = 9
        random.seed(42)
    ...
    def test_no_monolith_at_non_multiple_of_5(self) -> None:
        config = MiningConfig(system_id="breakstone")
        session = MiningSession(config)
        session.depth = 6
        random.seed(42)
    ...
    def test_monolith_hardness_scales_with_depth(self) -> None:
        ...
        # At depth 10: 10.0 + 10 * 0.5 = 15.0
        assert monolith.hardness == pytest.approx(15.0)
