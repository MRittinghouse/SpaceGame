...
COMMODITY_PROMPTS: dict[str, str] = {
    # --- Basic trade goods ---
    "food": (
        "A sealed ration container icon "
        + _BG_PREFIX
        + _VIEW
    ...
    "fuel": (
        "A glowing fuel cell canister icon "
        + _BG_PREFIX
        + _VIEW
    ...
    "rare_metals": (
        "A gleaming rare metal ingot icon "
        + _BG_PREFIX
        + _VIEW
    ...
    "exotic_goods": (
        "A sealed luxury container icon "
        + _BG_PREFIX
        + _VIEW
    ...
    "restricted_tech": (
        "A classified technology device icon "
        + _BG_PREFIX
        + _VIEW
    ...
    "combat_stims": (
        "A combat stimulant injector icon "
        + _BG_PREFIX
        + _VIEW
...
def generate_and_process(
    ...
    try:
        ...
        preview = resize_nearest(
            img, (NATIVE_SIZE[0] * PREVIEW_SCALE, NATIVE_SIZE[1] * PREVIEW_SCALE)
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-existing", action="store_true", help="Skip commodities that already have a .png"
    )
    ...
    final_count = (
        ...
        - len(list(output_dir.glob("*_raw.png")))
        - len(list(output_dir.glob("*_preview.png")))
    )
